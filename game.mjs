...
const fixIndexNumber = 1;
let randomNumber;
let correctWord = wordsList[getRandomNumber()];
...
function getRandomNumber() {
    randomNumber = Math.floor(Math.random() * (wordsList.length + fixIndexNumber)); 
    return randomNumber
}

function drawWordDisplay() {

    wordDisplay = emptySpace;

    for (let i = 0; i < numberOfCharInWord; i++) {
        if (guessedWord[i] != magicStringRemover.underScore) {
            wordDisplay += ANSI.COLOR.GREEN;
        }
        wordDisplay = wordDisplay + guessedWord[i] + magicStringRemover.space;
